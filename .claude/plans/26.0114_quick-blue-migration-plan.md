...
- [ ] 複製 `quick_blue_windows/lib` 到新專案
- [ ] 重命名主檔案：`quick_blue_windows.dart` → `quick_blue_plus_windows.dart`
- [ ] 更新類別名稱：
  - `QuickBlueWindows` → `QuickBluePlusWindows`
...
- [ ] 重命名類別：
  ```cpp
  // 舊: class QuickBlueWindowsPlugin
  // 新: class QuickBluePlusWindowsPlugin
  ```
- [ ] 更新 channel 名稱：
  ```cpp
  // Method channel
...
- [ ] 更新註冊函數：
  ```cpp
  // 舊: QuickBlueWindowsPluginRegisterWithRegistrar
  // 新: QuickBluePlusWindowsPluginRegisterWithRegistrar
  ```
- [ ] 檢查 Flutter 嵌入 API 兼容性：
  - [ ] `flutter::StreamHandler` 介面
...
- [ ] 更新 `CMakeLists.txt`:
  ```cmake
  cmake_minimum_required(VERSION 3.22)  # Flutter 3.24+ 要求
  set(PROJECT_NAME "quick_blue_plus_windows")
  project(${PROJECT_NAME} LANGUAGES CXX)

  set(PLUGIN_NAME "${PROJECT_NAME}_plugin")
  ```
- [ ] 檢查 C++/WinRT NuGet 套件版本：
  ```cmake
  # 可能需要升級版本
  ...
  ```
- [ ] 確認 C++ 標準版本（C++17 或 C++20）
- [ ] 更新目標連結配置

### 2.4 Windows API 兼容性測試

**工作項目：**

- [ ] 編譯測試（`flutter build windows`）
- [ ] 解決編譯錯誤（如有）
- [ ] 檢查 WinRT Bluetooth APIs：
  - `Windows.Devices.Bluetooth`
  - `Windows.Devices.Bluetooth.Advertisement`
  - `Windows.Devices.Bluetooth.GenericAttributeProfile`
- [ ] 驗證非同步操作（`IAsyncOperation`）
- [ ] 測試事件處理（`winrt::event_token`）
