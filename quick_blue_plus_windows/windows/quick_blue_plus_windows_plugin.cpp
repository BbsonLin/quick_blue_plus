#include "include/quick_blue_plus_windows/quick_blue_plus_windows_plugin.h"
...
class QuickBluePlusWindowsPlugin : public flutter::Plugin, public flutter::StreamHandler<EncodableValue> {
 public:
  static void RegisterWithRegistrar(flutter::PluginRegistrarWindows *registrar);

  QuickBluePlusWindowsPlugin();

  virtual ~QuickBluePlusWindowsPlugin();
 ...
 private:
  ...
  void QuickBluePlusWindowsPlugin::GattCharacteristic_ValueChanged(GattCharacteristic sender, GattValueChangedEventArgs args);
};

// static
void QuickBluePlusWindowsPlugin::RegisterWithRegistrar(
    flutter::PluginRegistrarWindows *registrar) {
  auto method =
      std::make_unique<flutter::MethodChannel<EncodableValue>>(
          registrar->messenger(), "quick_blue_plus/method",
          &flutter::StandardMethodCodec::GetInstance());
  auto event_scan_result =
      std::make_unique<flutter::EventChannel<EncodableValue>>(
          registrar->messenger(), "quick_blue_plus/event.scanResult",
          &flutter::StandardMethodCodec::GetInstance());
  auto message_connector_ =
      std::make_unique<flutter::BasicMessageChannel<EncodableValue>>(
          registrar->messenger(), "quick_blue_plus/message.connector",
          &flutter::StandardMessageCodec::GetInstance());

  auto plugin = std::make_unique<QuickBluePlusWindowsPlugin>();
...
QuickBluePlusWindowsPlugin::QuickBluePlusWindowsPlugin() {
  InitializeAsync();
}

QuickBluePlusWindowsPlugin::~QuickBluePlusWindowsPlugin() {}

winrt::fire_and_forget QuickBluePlusWindowsPlugin::InitializeAsync() {
  auto bluetoothAdapter = co_await BluetoothAdapter::GetDefaultAsync();
  bluetoothRadio = co_await bluetoothAdapter.GetRadioAsync();
}

void QuickBluePlusWindowsPlugin::HandleMethodCall(
    const flutter::MethodCall<flutter::EncodableValue> &method_call,
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
  ...
  if (method_name.compare("isBluetoothAvailable") == 0) {
    ...
    if (!bluetoothLEWatcher) {
      ...
      bluetoothLEWatcherReceivedToken = bluetoothLEWatcher.Received({ this, &QuickBluePlusWindowsPlugin::BluetoothLEWatcher_Received });
    }
...
void QuickBluePlusWindowsPlugin::BluetoothLEWatcher_Received(
    BluetoothLEAdvertisementWatcher sender,
    BluetoothLEAdvertisementReceivedEventArgs args) {
  SendScanResultAsync(args);
}

winrt::fire_and_forget QuickBluePlusWindowsPlugin::SendScanResultAsync(BluetoothLEAdvertisementReceivedEventArgs args) {
...
std::unique_ptr<flutter::StreamHandlerError<EncodableValue>> QuickBluePlusWindowsPlugin::OnListenInternal(
...
std::unique_ptr<flutter::StreamHandlerError<EncodableValue>> QuickBluePlusWindowsPlugin::OnCancelInternal(
    const EncodableValue* arguments)
{
  if (arguments == nullptr) {
    return nullptr;
...
winrt::fire_and_forget QuickBluePlusWindowsPlugin::ConnectAsync(uint64_t bluetoothAddress) {
  ...
  auto connnectionStatusChangedToken = device.ConnectionStatusChanged({ this, &QuickBluePlusWindowsPlugin::BluetoothLEDevice_ConnectionStatusChanged });
...
void QuickBluePlusWindowsPlugin::BluetoothLEDevice_ConnectionStatusChanged(BluetoothLEDevice sender, IInspectable args) {
...
void QuickBluePlusWindowsPlugin::CleanConnection(uint64_t bluetoothAddress) {
...
winrt::fire_and_forget QuickBluePlusWindowsPlugin::DiscoverServicesAsync(BluetoothDeviceAgent &bluetoothDeviceAgent) {
...
winrt::fire_and_forget QuickBluePlusWindowsPlugin::RequestMtuAsync(BluetoothDeviceAgent& bluetoothDeviceAgent, uint64_t expectedMtu) {
...
winrt::fire_and_forget QuickBluePlusWindowsPlugin::SetNotifiableAsync(BluetoothDeviceAgent& bluetoothDeviceAgent, std::string service, std::string characteristic, std::string bleInputProperty) {
  ...
  if (bleInputProperty != "disabled") {
    bluetoothDeviceAgent.valueChangedTokens[characteristic] = gattCharacteristic.ValueChanged({ this, &QuickBluePlusWindowsPlugin::GattCharacteristic_ValueChanged });
  } else {
    gattCharacteristic.ValueChanged(std::exchange(bluetoothDeviceAgent.valueChangedTokens[characteristic], {}));
  }
}

winrt::fire_and_forget QuickBluePlusWindowsPlugin::ReadValueAsync(BluetoothDeviceAgent& bluetoothDeviceAgent, std::string service, std::string characteristic) {
  auto gattCharacteristic = co_await bluetoothDeviceAgent.GetCharacteristicAsync(service, characteristic);
...
winrt::fire_and_forget QuickBluePlusWindowsPlugin::WriteValueAsync(BluetoothDeviceAgent& bluetoothDeviceAgent, std::string service, std::string characteristic, std::vector<uint8_t> value, std::string bleOutputProperty) {
  auto gattCharacteristic = co_await bluetoothDeviceAgent.GetCharacteristicAsync(service, characteristic);
...
void QuickBluePlusWindowsPluginRegisterWithRegistrar(
    FlutterDesktopPluginRegistrarRef registrar) {
  QuickBluePlusWindowsPlugin::RegisterWithRegistrar(
